...
class MyComponent(rio.Component):
    def handle_resize(self, event: rio.SizeEvent) -> None:
        global recorded_events
        print(
            f"Component resized to {event.component_width}x{event.component_height}"
        )
        recorded_events.append((event.component_width, event.component_height))
...
async def test_size_observer_reports_content_dimensions():
    """
    Test that SizeObserver reports its content's dimensions via on_resize and matches content layout.
    """
    global recorded_events
    ...
    size_observer = next(
        ...
        if isinstance(c, rio.SizeObserver)
    )
    rectangle = next(
        c
        for c in session._weak_components_by_id.values()
